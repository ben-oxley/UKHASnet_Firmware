...
int16_t gen_data(char *buf)
{
    ...
    for (int i = 0; i < 6; i++){
        int res = dht22_read(&tempdht, &humdht);
        // if (res==DHT_ERROR_ACK_TOO_LONG){
        //     sprintf(buf,"%sACK",buf);
        // }
        // if (res==DHT_ERROR_NOT_PRESENT){
        //     sprintf(buf,"%sNPRES",buf);
        // }
        // if (res==DHT_ERROR_SYNC_TIMEOUT){
        //     sprintf(buf,"%sESYNC",buf);
        // }
        // if (res==DHT_ERROR_DATA_TIMEOUT){
        //     sprintf(buf,"%sEDATA",buf);
        // }
        // if (res==DHT_ERROR_NONE){
        //     sprintf(buf,"%sGOOD",buf);
        // }
        // if (res==DHT_ERROR_CHECKSUM){
        //     sprintf(buf,"%sCSERR",buf);
        // }

        if (res==0){
            char* tempStrPos;
            char* humStrPos;
            tempStrPos = dtostrf(tempdht,7, 2, tempstr);
            while( *tempStrPos == ' ' )
                tempStrPos++;
            sprintf(buf, "%sT%sH%d", buf, tempStrPos,(int)humdht);
            break;
        }
    }
    // dht_read_data(&dhtSensor, &tempdht, &humdht);
    // dht_read_data(&dhtSensor, &tempdht, &humdht);
    // dht_read_data(&dhtSensor, &tempdht, &humdht);
    // dht_read_data(&dhtSensor, &tempdht, &humdht);
    // dht_read_data(&dhtSensor, &tempdht, &humdht);
